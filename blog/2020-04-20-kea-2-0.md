...
:::note
This is **a draft**. It'll go out together with Kea 2.0 and the new docs site.

Try kea `2.0.0-beta.4` to test the new goodies today!
:::

7 months after the release of Kea 1.0 I'm proud to announce version 2.0!

This version brings several *convenience features*. It's a rather small release, yet there
were a few breaking changes (none in `kea()` calls!), which warranted a new major version.

What changed? Read below! 

But first! You are reading the brand new documentation site for Kea! Powered by [docusaurus v2](https://v2.docusaurus.io/)!
Over *10000 new words* were written for these docs in an effort to really clarify
how Kea works. 

Start with the [quickstart](/docs/quickstart) if you're new to Kea. Then head on to the  [core concepts](/docs/concepts). 
Please also read them if you've been using Kea for a while. You might learn something you didn't know!
Then check out [advanced concepts](/docs/advanced), [debugging](/docs/debugging) and other pages for more. 
...
For years Kea has supported two different side effect libraries: [sagas](/docs/effects/sagas) and 
[thunks](/docs/effects/thunks).

With Kea 1.0, I added a new lightweight one called [listeners](/docs/effects/listeners).
...
**Please note:** If you were using listeners with Kea 1.0, make sure to remove `listenersPlugin` from 
...
* use `counterLogic.something` as a *key* in `reducers` or `listeners` 
* use `counterLogic.selectors` in `selectors`
* use `counterLogic.anything` *inside* a `listener`
...
inside a listener. For that please read the section in the [Advanced Concepts](http://localhost:3001/docs/advanced#calling-mount-inside-listeners-with-autoconnect-true) 
page.
