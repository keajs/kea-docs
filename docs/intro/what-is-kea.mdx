...
To illustrate how this works, we will create a `loginLogic` with one action, `setUsername`, and one value, exposed through a `username` reducer:
...
or with [hooks](/docs/hooks/) in a React component, to tie its `mount`/`unmount` lifecycle to the component.
...
There's no magic behind logic builders themselves either. They just return a function that adds things onto a `logic`. For example:
...
Here's a `setters({ username: 'keajs' })` logic builder that combines the `setUsername` action and `username` reducer from above into one call:
...
You can build a complete webapp in Kea's data layer, leaving React to handle the views.
...
within Kea's _data layer_, leaving React to do an amazing job as a _view layer_.
...
Kea discourages the use of `Context` because you shouldn't re-render everything.
...
Kea thinks layers shield complexity.
