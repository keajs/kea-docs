...
## Heavy state management for React

Kea is a complete framework for managing application state in [React](https://reactjs.org/) webapps.

Kea's unit of organisation is a `logic`, which is a pretty _big and opinionated unit of state_, when compared to the atoms and molecules some other systems use.

A `logic` is created by passing an array of logic builders to [`kea([])`](/docs/core/kea). Some core logic builders are: [`actions`](/docs/core/actions), [`reducers`](/docs/core/reducers), [`listeners`](/docs/core/listeners), and [`selectors`](/docs/core/selectors).

To illustrate how this works, we will create a `userLogic` with one action, `setUsername`, and one value, exposed through a `username` reducer:
...
We can use the logic directly:

```ts
userLogic.mount()
userLogic.actions.setUsername('posthog')
userLogic.values.username === 'posthog'
```

or use it in a React component:

```tsx
import { useValues, useActions } from 'kea'

function Login() {
  const { setUsername } = useActions(userLogic)
  const { username } = useValues(userLogic)
  // ...
}
```
...
There's no magic behind logic builders. They just return a function that adds things onto a `logic`. For example:
...
You can combine logic builders to abstract common patterns.

Let's abstract away that pesky `username` reducer + `setUsername` action pattern into a simple `setters` logic builder:
...
// same as before
const userLogic = kea([
  setters({ username: 'posthog' }), // much shorter
])
userLogic.mount()
userLogic.actions.setUsername('posthog')
userLogic.values.username === 'posthog'
...
We have already built higher level abstractions for [`routing`](/docs/core/router), [`forms`](/docs/core/forms) and [`loaders`](/docs/core/loaders), to name a few.

## Data layer

When fully deployed, Kea acts as a separate [data layer](/blog/data-first-frontend-revolution), which communicates with
React via `actions` and `values`.

Kea discourages the use of `useState` and `useEffect`. We believe that it's hard enough to get your data layer right,
so why burden yourself even more, by hosting it in a DOM-like tree of React nodes?

<KeaReact />

Instead, keep all your data in their own layer, where its free to communicate with itself.

---


Kea is the missing piece of the frontend puzzle. It's the backend of the frontend.
