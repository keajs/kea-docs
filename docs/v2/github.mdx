...
To follow along, you must have a good understanding of React and you _should_ have gone through
[the quickstart](/docs/BROKEN) or [core concepts](/docs/BROKEN) pages at
least once.
...
In Kea, everything starts with an [**action**](/docs/BROKEN). Every button press, every change in a textfield,
...
To store data you use a [**reducer**](/docs/BROKEN). Reducers are functions that react to actions and change
...
The next step is to use a [**listener**](/docs/BROKEN) to listen for the `setUsername` action and run some code
...
To load the repositories on page load, we can hook into the `afterMount` [event](/docs/BROKEN)
...
Luckily listeners come with one very cool feature: [breakpoints](/docs/BROKEN).
...
[**selector**](/docs/BROKEN) to sort it dynamically and automatically.
...
I opted to avoid it in the example above. See the [listeners](/docs/BROKEN)
...
:::note Next Steps
Read the [Core Concepts](/docs/BROKEN) guide to get an in-depth understanding of how Kea works
